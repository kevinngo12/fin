    ...
    $("#submit-form-max-interest").on("click", function () {
        ...
        if (e.length >= 6 && $(".field-error").length < 1) {
            ...
            if ($(".other-field").is(":visible") && $("#request-other").val().trim() !== "") {
                ...
                // Add "(teams)" or " (teams)" to sourceOtherRadio and sourceOther
                if ($("#sourceOtherRadio").val().trim() === "__other_option__") {
                    $("#sourceOtherRadio").val("__other_option__ (teams)");
                } else {
                    $("#sourceOtherRadio").val($("#sourceOtherRadio").val() + " (teams)");
                }
                if ($("#sourceOther").val().trim() === "") {
                    $("#sourceOther").val("(teams)");
                } else {
                    $("#sourceOther").val($("#sourceOther").val() + " (teams)");
                }
            }
            ...
            if ($("#traffic-source").is(":visible") && $("#request-traffic-source-volume").text() !== "-") {
                ...
                // Add "(teams)" or " (teams)" to sourceOther
                if ($("#sourceOther").val().trim() === "") {
                    $("#sourceOther").val("(teams)");
                } else {
                    $("#sourceOther").val($("#sourceOther").val() + " (teams)");
