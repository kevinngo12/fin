...
(function() {
  ...
  function setXAxis(chart) {
    ...
    for(var i = 0; i < xLabels.length; i++) {
      ...
      var labelClasses = (chart.options.xAxis && chart.options.xAxis.labels) ? 'chart__axis-label chart__axis-label--x js-chart__axis-label' : 'cl8-hide js-chart__axis-label';
  ...
  function setYAxis(chart) {
    ...
    for(var i = yLabels.length - 1; i >= 0; i--) {
      var textEl = document.createElementNS('http://www.w3.org/2000/svg', 'text');
      var labelClasses = (chart.options.yAxis && chart.options.yAxis.labels) ? 'chart__axis-label chart__axis-label--y js-chart__axis-label' : 'cl8-hide js-chart__axis-label';
      setAttributes(textEl, {class: labelClasses, 'alignment-baseline': 'middle'});
    ...
    for(var i = 1; i < yLabels.length; i++ ) {
      ...
      if(hideGuides) {
        setAttributes(rectEl, {class: 'chart__guides js-chart__guides cl8-opacity-0'});
      }
  ...
  function createSrTables(chart) {
    ...
    var table = '<div class="chart__sr-table cl8-sr-only">';
    for(var i = 0; i < chart.options.datasets.length; i++) {
  ...
  function initTooltips(chart) {
    ...
    if(chart.options.yIndicator) {
      ...
      setAttributes(tooltipLine, {x1: 0, y1: chart.topDelta, x2: 0, y2: chart.topDelta + chart.yAxisHeight, transform: 'translate('+chart.leftDelta+' '+chart.topDelta+')', class: 'chart__y-indicator js-chart__y-indicator cl8-hide'});
    ...
    if(chart.tooltipOn) {
      ...
      tooltip.setAttribute('class', 'chart__tooltip js-chart__tooltip cl8-hide '+chart.tooltipClasses);
  ...
  function resetTooltip(chart) {
    ...
    if(chart.tooltipOn) chart.tooltip.classList.add('cl8-hide');
    if(chart.options.yIndicator) chart.interLine.classList.add('cl8-hide');
    resetMarkers(chart, false);
    resetBars(chart, false);
